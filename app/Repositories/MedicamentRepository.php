...
class MedicamentRepository
{
  ...
  {
    $guard = Medicament::where(
      'denomination',
      '=',
      $request->input('denomination')
    )->get();
    if (count($guard) > 0) {
      throw new \Exception('Un médicament portant ce nom existe déjà. ');
    }

    $this->medicamentCustom = $this->_populateModelFromForm(
      $request,
      $this->medicamentCustom
    );
   ...
   */
  private function _populateModelFromForm(
    Request $request,
    Medicament $medicament
  ) {
    $medicament->denomination = $request->input('denomination');

    $medicament->custom_indications = json_encode(
      $request->input('custom_indications')
    );

    $medicament->conservation_frigo = $request->input('conservation_frigo');

    $medicament->conservation_duree = json_encode(
      $request->input('conservation_duree')
    );

    $medicament->voies_administration = json_encode(
      $request->input('voies_administration')
    );
  ...
  {
    $cible = $this->getCible(
      $commentaire['cible_id'],
      $this->medicamentCustom->id
    );
  ...
  public function getCible($cible, $medicament_id)
  {
    ...
    switch ($substanceOrMedicament[0]) {
      ...
      case 'S':
        $composition = is_numeric($substanceOrMedicament[1])
          ? Composition::findOrFail($substanceOrMedicament[1])
          : Composition::where(
            'denomination',
            '=',
            $substanceOrMedicament[1]
          )->firstOrFail();
        return [
          'type' => 'S',
          'model' => $composition,

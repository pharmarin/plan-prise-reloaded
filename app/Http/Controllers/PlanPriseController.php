...
class PlanPriseController extends Controller
{
  ...
  public function export($pp_id)
  {
      ...
      foreach ($group['inputs'] as $input) {
        if ($group_id === 'posologies') {
          if (
            ...
            !(
              isset($settings->inputs->{$input['id']}) &&
              $settings->inputs->{$input['id']}->checked === true
            )
          ) {
            continue;
          }
    ...
    $medicaments = array_map(function ($medicament) use (
      $custom_data,
      $columns
    ) {
    ...
    $default);
    if (
      count(array_filter(array_column($medicaments, 'conservation_duree'))) ===
      0
    ) {
    ...
    $pdf = PDF::loadView(
      'plan-prise.print',
      compact('pp_id', 'medicaments', 'columns')
    );
  ...
  public function print($pp_id)
  {
    ...
    return view('plan-prise.print')->with(
      compact('pp_id', 'medicaments', 'columns')
    );
  }
}

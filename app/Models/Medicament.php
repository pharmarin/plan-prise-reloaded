...
class Medicament extends Model
{
  ...
  public function getConservationDureeAttribute($conservation_array)
  {
    ...
    if (!$conservation_array) {
      return;
    }
    return array_filter(
      array_map(function ($conservation) {
        return $conservation->laboratoire === null &&
          $conservation->duree === null
          ? null
          : $conservation;
      }, $conservation_array)
    );
  ...
  public function getVoiesAdministrationStringAttribute()
  {
    if ($voiesAdministrationArray = json_decode($this->voies_administration)) {
      ...
      return array_map(function ($voieAdministration) use (
        $voiesAdministrationValues
      ) {
        return $voiesAdministrationValues[
          $voieAdministration->voies_administration
        ];
      },
  ...
  public function getEmpty()
  {
    return tap($this, function ($medicament) {
      $medicament->forceFill(
        array_fill_keys(
          array_merge(
            \Schema::getColumnListing($medicament->getTable()),
            $this->appends,
            ['bdpm']
          ),
          null

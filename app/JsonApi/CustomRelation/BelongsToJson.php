...
class BelongsToJson extends AbstractRelationshipAdapter implements
  HasManyAdapterInterface
{
  ...
  public function update(
    ...
    if ($related = $this->findToMany($relationship)) {
      $relation->sync($this->extractIds($related))->save();
    } else {
      $relation->sync([])->save();
    }
  }
  ...
  {
    return collect((array) $models)->map(function ($model) {
      return $model->id;
    });
  }
}

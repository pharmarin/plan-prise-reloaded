...
                    <a href="#" className={"list-group-item list-group-item-action py-2" + noBorderClass} onClick={(e) => this.handleSearchSelect(e, result.codeCIS, selected)} >
                      {
                        this.props.multiple ? <input type="checkbox" checked={selected} className="form-check-input d-none mt-1" onChange={(e) => this.handleSearchSelect(e, result.codeCIS, selected)} id={result.codeCIS}/> : null
                      }
                      <label className={"form-check-label" + (this.state.selected.includes(Number(result.codeCIS)) ? " font-weight-bold" : "")} htmlFor={result.codeCIS}>
                      <div className="d-flex">
                      <div>{checkboxIcon}</div>
                      <div className="text-truncate flex-fill">{result.denomination}</div>
                      <div>({result.codeCIS})</div> {this.isSuppressed(result.codeCIS) ? "Supprimé" : ""}

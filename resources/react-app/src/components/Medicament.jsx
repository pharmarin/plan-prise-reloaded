...
class Medicament extends React.Component {
  constructor(props) {
    ...
    for (const key in this.newInputs) {
      if (this.newInputs.hasOwnProperty(key)) {
        this.newInputs[key].defaultValue = {
          ...this.newInputs[key].emptyObject,
        };
      }
    }
    ...
    if (window.php.old_medicament) {
      ...
      const { old_medicament } = window.php;
    ...
    try {
      ...
      switch (inputName) {
        ...
        case 'conservation_duree':
          result = [];
          for (const [labo, duree] of Object.entries(string)) {
            result.push({
              laboratoire: labo,
              duree,
            });
  ...
  render() {
    return (
      <MedicamentForm
        ...
        medicament={this.medicament_edit}
        method={this.method}
        {...this.props}
      />
    );

...
export default class MedicamentForm extends React.Component {
  constructor(props) {
    ...
    if (Array.isArray(this.state.commentaires)) {
      this.state.commentaires = this._getCommentairesObject(
    ...
    if (this.props.medicament) {
      if (
        this.props.medicament.bdpm &&
        this.props.medicament.bdpm[0] !== undefined
      ) {
        this.state.bdpm = this.props.medicament.bdpm;
      }
      if (this.props.medicament.compositions) {
        this.state.composition_array = this.props.medicament.compositions.map(
          (composition) => {
            return {
        ...
        this.state.inputs = this._getSubstancesActivesObject(
          this.state.composition_array,
          this.state.inputs
        );
      }
    }
  ...
  _getSubstancesActivesObject = (composition_array, inputs_options) => {
    ...
    if (composition_array) {
      composition_array.forEach((composition) =>
    ...
    return old_commentaires
      ...
      .map((commentaire) => {
        ...
        if (!commentaire) {
          ...
          new_commentaire.cible_id =
            new_commentaire.cible_type === 'App\\Models\\Composition'
    ...
    return (
      <div className="card">
        ...
        <div className="card-body">
          <h6>
            Correspondance dans la Base de Données Publique des Médicaments
          </h6>
          <ul className="list-unstyled">
            <li key="add">
              <Search
                ...
                defaultValue={this.state.inputs.denomination.defaultValue}
              ...
              this.state.bdpm.map((medicament, index) => (
                <li key={index}>
                  <button
                    ...
                    onClick={(e) => this.removeAPILine(e, medicament.code_cis)}
                  >
                    <i className="fa fa-minus-circle"></i>
                  </button>{' '}
                  {medicament.denomination} (
                  <a
                    href={`http://base-donnees-publique.medicaments.gouv.fr/affichageDoc.php?specid=${medicament.code_cis}&typedoc=R`}
                    target="_blank"
                  >
                    {medicament.code_cis}
                  </a>
            ...
            <>
              <form action={window.php.routes.action} method="POST">
                ...
                <SearchComposition
                  ...
                  onCompositionChange={(composition_array) =>
                    this.setState({
                      composition_array: composition_array,
                      inputs: this._getSubstancesActivesObject(
                        composition_array,
                        this.state.inputs
                      ),
                    })
                  }
                  onPrecautionChange={(precaution_array) =>
                    this.setState({
                      commentaires: this._getCommentairesObject(
                        this.state.commentaires,
                        precaution_array,
                        true
                      ),
                ...
                {Object.keys(this.state.inputs).map((inputName) => (
                  <MedicamentInput
                    ...
                    setState={(newState) =>
                      this.setState({ [inputName]: newState })
                    }
                    voiesAdministration={this.state.voies_administration}
                    deleteCallback={(prec) =>
                      this.setState({
                        deleted_precautions: this.state.deleted_precautions.concat(
                          prec
                        ),
                      })
                    }
                ...
                <button
                  ...
                  disabled={!(this.state.composition_array.length > 0)}
                >
                  {submitButton}
                </button>
              </form>
